...
const PlusScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    console.log('PlusContext:', plusCtx);
  }, []);

  if (!plusCtx || typeof plusCtx.purchasePlus !== 'function') {
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
        <Text>Loading Plus context...</Text>
      </View>
    );
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Text style={{ color: 'blue', textAlign: 'center', marginTop: 24 }}>
  ...
  const handleResistanceInput = (val) => {
	...
	const processImage = async (uri) => {
		try {
			...
			if (usePreprocessing) {
				jpegUri = await preprocessImage(jpegUri);
			}
			...
			console.log('OCR: valid file path:', jpegUri);
			...
			const cleanedLines = resultLines
				.map(line => postprocessOcrText(line))
			...
			if (!napeti || !proud || !odpor) {
				...
				if (allNums && allNums.length > 0) {
					...
					if (nNums.length >= 3) {
						let max = nNums[0], min = nNums[0];
						nNums.forEach(x => {
							if (x > max) max = x;
							if (x < min) min = x;
						});
						const mid = nNums.find(x => x !== max && x !== min) || '';
			...
			console.log('OCR: fatal try/catch failed', error);
