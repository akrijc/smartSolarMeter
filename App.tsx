...
import { PlusProvider, PlusContext } from './src/context/PlusContext';
...
import OCRSettingsScreen from './src/screens/ocrSet';
import { OCRSettingsProvider } from './src/context/OCRSettingsContext';
...
import { OCRSettingsContext } from './src/context/OCRSettingsContext';
...
async function getJpegFileUri(inputUri) {
  // Pokud už je file:// a končí na .jpg/.jpeg, nemusíme převádět
  if (
    inputUri.startsWith('file://') &&
    (inputUri.toLowerCase().endsWith('.jpg') || inputUri.toLowerCase().endsWith('.jpeg'))
  ) {
    return inputUri;
  }

  // Jinak vždy provedeme ImageManipulator
  try {
    ...
    throw new Error('Nepodařilo se převést obrázek na JPG: ' + e.message);
  }
}
...
const ExportPreview = ({ children, style }) => (
  <ScrollView
    horizontal
...
const SettingsProvider = ({ children }) => {
  ...
  const [customRangeOrder, setCustomRangeOrder] = useState('BLOCKS'); // BLOCKS | ZIP
    ...
    const loadSettings = async () => {
      ...
      try {
        ...
        if (cr !== null && cr !== undefined) setCustomCableRange(cr);
			...
			try {
				  const ro = await AsyncStorage.getItem('@custom_range_order');
				  if (ro === 'BLOCKS' || ro === 'ZIP') setCustomRangeOrder(ro);
			} catch {}
  ...
  useEffect(() => {
    if (!loading) AsyncStorage.setItem('@cable_count', String(cableCount)).catch(() => {});
 	...
 	useEffect(() => {
		if (!loading) {
			AsyncStorage.setItem('@custom_range_order', customRangeOrder).catch(() => {});
		}
	}, [customRangeOrder, loading]);
  ...
  useEffect(() => {
    if (!loading) AsyncStorage.setItem('@custom_range_order', customRangeOrder).catch(() => {});
  }, [customRangeOrder, loading]);
...
const colors = {
  ...
  // UI tokens (místo hardcoded #666/#888/#bbb)
  mutedText: '#6B7280',
  disabledBg: '#9CA3AF',
};

// --- Force readable Text color even in Android dark mode ---
Text.defaultProps = Text.defaultProps || {};
const prevStyle = Text.defaultProps.style;
...
const preprocessImage = async (uri) => {
	...
	try {
		...
		// 2. Grayscale – zjednoduší detekci textu
		manipResult = await ImageManipulator.manipulateAsync(
			manipResult.uri,
			[{ grayscale: true }],
			{ compress: 0.95, format: ImageManipulator.SaveFormat.JPEG }
		...
		// 3. Zesílení kontrastu + mírné ztmavení (odstraní modré pozadí)
		manipResult = await ImageManipulator.manipulateAsync(
			manipResult.uri,
			[
				{ contrast: 1.9 },     // výrazně zvýšený kontrast
				{ brightness: -0.15 }  // lehce snížený jas – modré pozadí se ztratí
			],
			{ compress: 0.9, format: ImageManipulator.SaveFormat.JPEG }
		);

		// 4. Ostření (pokud by to bylo podporováno – volitelné)
		// manipResult = await ImageManipulator.manipulateAsync(
		// 	manipResult.uri,
		// 	[{ sharpen: 1 }],
		// 	{ compress: 0.9, format: ImageManipulator.SaveFormat.JPEG }
...
const postprocessOcrText = (raw) => {
	...
	// ✨ Nové řádky (doporučené v návodu):
	text = text.replace(/[|!¡?¿;‘’“”"(){}\[\]\/\\]/g, ''); // odstranění speciálních znaků
	text = text.replace(/[lI]/g, '1'); // OCR chyba l/I → 1
	text = text.replace(/[\u2013\u2014]/g, '-'); // různé pomlčky → "-"

	// ✅ Tvoje stávající nahrazování:
	text = text.replace(/[\u00B0]/g, '');
...
const PlusScreen = ({ navigation }) => {
  ...
  const isPlus = !!plusCtx.plus;
  const restorePurchase = () => plusCtx.restorePlus && plusCtx.restorePlus();
  ...
  return (
    ...
    >
      <ScrollView
        style={{ flex: 1, padding: 24, backgroundColor: colors.surface }}
        contentContainerStyle={{ backgroundColor: colors.surface }}
        keyboardShouldPersistTaps="handled"
      >
        <Text style={{ fontWeight: 'bold', marginBottom: 8, fontSize: 16, color: '#111' }}>
          ...
          {benefits.map((b, i) => (
            <View key={i} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 6 }}>
              <Ionicons
                ...
                style={{ marginRight: 7 }}
              />
              <Text style={{ fontSize: 15, color: '#111' }}>{b}</Text>
            </View>
          ))}
        </View>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.86}
        >
          <Ionicons name="diamond" size={22} color="#fff" style={{ marginRight: 8 }} />
          <Text style={localStyles.plusButtonText}>
        ...
        >
          ...
          <Text style={localStyles.plusButtonText}>
            {t('Restore Purchase', { defaultValue: 'Obnovit nákup' })}
          </Text>
        </TouchableOpacity>

        {isPlus && (
          <>
            <Text style={{ color: colors.accent, marginTop: 12, marginBottom: 10 }}>
              {t('youHavePlusVersion', { defaultValue: 'You have Plus version' })}
            </Text>

            <TouchableOpacity
              style={[localStyles.plusButton, { backgroundColor: '#222' }]}
              onPress={plusCtx.openManageSubscription}
              activeOpacity={0.86}
            >
              <Ionicons name="settings-outline" size={20} color="#fff" style={{ marginRight: 8 }} />
              <Text style={localStyles.plusButtonText}>
                {t('manageSubscription', { defaultValue: 'Správa předplatného' })}
              </Text>
            </TouchableOpacity>
          </>
        )}
        ...
        <TouchableOpacity
          style={[localStyles.plusButton, { backgroundColor: colors.inputBorder, marginTop: 24 }]}
          onPress={() => navigation.goBack()}
          activeOpacity={0.86}
        >
          <Ionicons name="arrow-back" size={22} color={colors.primary} style={{ marginRight: 8 }} />
          <Text style={[localStyles.plusButtonText, { color: colors.primary }]}>
            {t('Back', { defaultValue: 'Back' })}
          </Text>
        </TouchableOpacity>

        <TouchableOpacity
          onPress={() => Linking.openURL('https://akrijc.github.io/smartsolarmeter-privacy/terms-of-use.html')}
        >
          <Text style={{ color: '#007aff', textAlign: 'center', marginTop: 24 }}>
            {t('termsOfUse', { defaultValue: 'Podmínky použití' })}
          </Text>
        </TouchableOpacity>

        <TouchableOpacity
          onPress={() => Linking.openURL('https://akrijc.github.io/smartsolarmeter-privacy/privacy-policy.html')}
        >
          <Text style={{ color: '#007aff', textAlign: 'center', marginBottom: 12 }}>
            {t('privacyPolicy', { defaultValue: 'Zásady ochrany soukromí' })}
          </Text>
        </TouchableOpacity>
...
const generateCSVExportMultipleColumns = (data) => {
  ...
  const colNames = [
    ...
    _export(data.exportT('csv_header_errors', { defaultValue: data.exportT('Errors', { defaultValue: 'Errors' }) })),
  ];
...
function normalizeInput(str) {
  return (str || '')
    .replace(/[\u2013\u2014–—]/g, '-') // en dash/em dash na '-'
    .replace(/[,;]+/g, ',')           // více čárek/oddělovačů na čárku
    .replace(/\s*-\s*/g, '-')         // mezery kolem pomlčky pryč
    .replace(/\s+/g, ' ')             // více mezer na jednu
    .replace(/,\s*/g, ',')            // mezery za čárkou pryč
    .trim();
}

function parseLabel(label) {
  const m = String(label).trim().match(/^([A-Z]+)(\d+)(?:\.(\d+))?$/i);
  if (!m) throw new Error('Invalid label: ' + label);
  return {
    letters: m[1].toUpperCase(),
    main: parseInt(m[2], 10),
    sub: m[3] !== undefined ? parseInt(m[3], 10) : undefined,
  };
}

function labelToString(obj) {
  return obj.letters + obj.main + (obj.sub !== undefined ? '.' + obj.sub : '');
}
...
function compareLabels(a, b) {
  const cmpLetters = lettersToNumber(a.letters) - lettersToNumber(b.letters);
  if (cmpLetters !== 0) return cmpLetters;
  const cmpMain = a.main - b.main;
...
function nextLabel(curr, end) {
  // Postup: nejdřív letters, pak main, pak sub – jen pokud je to potřeba k dosažení end
  if (curr.letters !== end.letters) {
    const n = lettersToNumber(curr.letters);
    const nEnd = lettersToNumber(end.letters);
    if (n < nEnd) {
      return { letters: numberToLetters(n + 1), main: curr.main, sub: curr.sub };
    }
  }

  if (curr.main !== end.main) {
    if (curr.main < end.main) {
      return { letters: curr.letters, main: curr.main + 1, sub: curr.sub };
    }
  }

  if ((curr.sub ?? 0) !== (end.sub ?? 0)) {
    const nextSub = (curr.sub ?? 0) + 1;
    return { letters: curr.letters, main: curr.main, sub: nextSub };
  }

  return end;
}

function expandRange(start, end, maxCount = 10000, order = 'BLOCKS') {
  // order:
  //  - 'BLOCKS' => A1,A2..,B1,B2..  | pro sub: A1.1..A1.20, B1.1..B1.20...
  //  - 'ZIP'    => A1,B1,C1,A2...   | pro sub: A1.1,B1.1,C1.1,...,A1.2,B1.2...

  const cmp = compareLabels(start, end);
  if (cmp > 0) throw new Error('Range start > end');

  const out = [];
  const push = (letters, main, sub) => {
    out.push(labelToString({ letters, main, sub }));
    if (out.length >= maxCount) throw new Error('Range too big');
  };

  const startL = lettersToNumber(start.letters);
  const endL = lettersToNumber(end.letters);

  const hasStartSub = start.sub !== undefined && start.sub !== null;
  const hasEndSub = end.sub !== undefined && end.sub !== null;

  // 1) Sub-range jen v rámci stejného letters+main: A1.1-A1.20
  if (
    start.letters === end.letters &&
    start.main === end.main &&
    hasStartSub &&
    hasEndSub
  ) {
    if (start.sub > end.sub) throw new Error('Range start > end');
    for (let s = start.sub; s <= end.sub; s++) {
      push(start.letters, start.main, s);
    }
    return out;
  }

  // 2) "Mřížka" přes písmena + sub při stejném main: A1.1-D1.20
  //    (tzn. letters se mění, main je fixní, sub je rozsah)
  if (
    start.main === end.main &&
    hasStartSub &&
    hasEndSub
    ...
    const subStart = start.sub;
    const subEnd = end.sub;
    if (subStart > subEnd) throw new Error('Range start > end');

    if (order === 'ZIP') {
      // A1.1,B1.1,C1.1,...,A1.2,B1.2,...
      for (let s = subStart; s <= subEnd; s++) {
        for (let ln = startL; ln <= endL; ln++) {
          push(numberToLetters(ln), main, s);
        }
      }
    } else {
      // BLOCKS: A1.1..A1.20,B1.1..B1.20,...
      for (let ln = startL; ln <= endL; ln++) {
        const letters = numberToLetters(ln);
        for (let s = subStart; s <= subEnd; s++) {
          push(letters, main, s);
        }
      }
  ...
  // 3) Klasický "mřížkový" range bez sub: A1-C4
  //    (sub musí být u obou prázdné)
  if (hasStartSub || hasEndSub) {
    // Smíchané tvary typu A1.1-C4 nebo A1-C4.2 nepodporujeme (aby to nedělalo bordel)
    throw new Error('Mixed sub/non-sub range not supported');
  }

  const startMain = start.main;
  const endMain = end.main;

  if (order === 'ZIP') {
    // A1,B1,C1,A2,B2,C2...
    for (let m = startMain; m <= endMain; m++) {
      for (let ln = startL; ln <= endL; ln++) {
        push(numberToLetters(ln), m, undefined);
      }
    }
  } else {
    // BLOCKS: A1,A2..,B1,B2..
...
function parseToken(token) {
  ...
  if (dashIdx === -1) {
    return { type: 'single', label: t.toUpperCase() };
  }
  const start = t.slice(0, dashIdx);
  const end = t.slice(dashIdx + 1);
  return { type: 'range', start: parseLabel(start), end: parseLabel(end) };
}

function getCustomCableLabels(rangeStr, maxCount = 10000, maxTimeMs = 500, order = 'BLOCKS') {

  const norm = normalizeInput(rangeStr);
  ...
  for (let tok of tokens) {
    ...
    if (parsed.type === 'single') {
      ...
      let expanded;
      try {
        expanded = expandRange(parsed.start, parsed.end, maxCount - labels.length, order);
      } catch (e) {
        continue;
      }
      ...
      for (const l of expanded) {
        ...
        if (Date.now() - started > maxTimeMs) break;
      }
    }
...
function getCableLabels(mode, count, customRange, numericRange, customRangeOrder = 'BLOCKS') {
  ...
  if (mode === 'A') {
    ...
    return getNumericLabels(numericRange, count);
		...
		try {
			const labels = getCustomCableLabels(customRange, 10000, 500, customRangeOrder);
			if (labels.length > 500) return [];
			return labels.slice(0, count);
		} catch (e) {
			return [];
		}
...
function SettingsScreen({ navigation }) {
  ...
  const languageOptions = ['cs','en','nl','zh','de','es','fr'];
	...
	// --- Custom Range validace (debounce) ---
	...
	useEffect(() => {
		...
		debounceTimeout.current = setTimeout(() => {
			if (settings.cableMode === 'custom' && plusCtx.plus) {
				const v = (settings.customCableRange || '').trim();
				...
				let labels = [];
				try {
					labels = getCustomCableLabels(v, 1001, 200, settings.customRangeOrder);
				} catch (e) {
					labels = [];
				}

				// ROZPRACOVANÝ INPUT → žádná chyba
				if (!labels || labels.length === 0) {
					if (/[^-]-$/.test(v) || /[A-Z]+\d+\.?$/i.test(v)) {
						setCustomRangeError('');
					} else {
						setCustomRangeError(
							t('customRangeError', {
								defaultValue: 'Chybný format znaceni.',
							})
						);
					}
				} else if (labels.length > 500) {
					setCustomRangeError(
						t('customRangeTooBig', {
							defaultValue: 'Rozsah je prilis velky (max 500 polozek).',
						})
					);
				} else {
					setCustomRangeError('');
    ...
    >
      <ScrollView contentContainerStyle={{ padding: 24 }}>
        ...
        <View style={{ flexDirection: 'row', marginBottom: 6, flexWrap: 'wrap' }}>
          ...
          {!plusCtx.plus && (
            <>
              <TouchableOpacity style={[styles.badButton, { backgroundColor: colors.disabledBg }]} disabled={true}>
        ...
        <View style={styles.formGroup}>
          ...
          <TextInput style={[styles.input, { color: colors.text }]} keyboardType="numeric" value={settings.cableCount === 0 ? '' : String(settings.cableCount)} onChangeText={v => { if (v === '') { settings.setCableCount(0); } else { let value = Number(v.replace(/[^0-9]/g, '')); if (isNaN(value)) value = 0; if (value < 0) value = 0; if (value > maxCableCount) value = maxCableCount; settings.setCableCount(value); } }} onBlur={() => { if (!settings.cableCount || isNaN(settings.cableCount) || settings.cableCount < 1) { settings.setCableCount(1); } }} placeholderTextColor={colors.mutedText} />
            ...
            <Text style={{ color: colors.mutedText, fontSize: 12, marginTop: 2 }}>
              {t('unlimitedPlus', { defaultValue: 'Ve verzi Plus neomezený počet.' })}
				...
				{settings.cableMode === 'custom' && plusCtx.plus && (
					<View style={styles.formGroup}>
						<Text>
							{t('customRangeLabel', { defaultValue: 'custom range (např. A1-F6)' })}
						</Text>

						<TextInput style={[styles.input, { color: colors.text }]} value={settings.customCableRange} onChangeText={settings.setCustomCableRange} placeholder={t('customRangeLabel', { defaultValue: 'custom range (např. A1-F6)' })} placeholderTextColor={colors.mutedText} autoCapitalize="characters" />
						...
						<Text style={[styles.hintText, { marginTop: 8 }]}>
							{t('customRangeHelpTitle', {
								defaultValue: 'Jak zadat vlastní rozsah:',
							})}
						</Text>

						<Text style={[styles.hintText, { marginTop: 4 }]}>
							{t('customRangeHelpExamples', {
								defaultValue:
									'Příklady: A1-F6 • AA1-AC5 • A1.1-D1.20',
							})}
						</Text>

						<Text style={[styles.hintText, { marginTop: 4 }]}>
							{t('customRangeHelpMix', {
								defaultValue:
									'Můžeš mixovat čárkami: A1-A3, C1, B2.1-B2.8',
							})}
						</Text>

						<View style={{ marginTop: 14 }}>
							<Text style={{ fontWeight: '700', marginBottom: 8 }}>
								{t('customRangeOrderTitle', {
									defaultValue: 'Pořadí generování',
								})}
							</Text>

							<View style={{ flexDirection: 'row', gap: 10 }}>
								<TouchableOpacity
									onPress={() => settings.setCustomRangeOrder('BLOCKS')}
									style={[
										styles.badButton,
										{
											flex: 1,
											backgroundColor:
												settings.customRangeOrder === 'BLOCKS'
													? colors.accent
													: colors.disabledBg,
											flexDirection: 'row',
											alignItems: 'center',
								...
								>
									...
									<View>
										<Text style={styles.badButtonText}>
											{t('customRangeOrderBlocksTitle', {
												defaultValue: 'Po řadě',
											})}
										</Text>
										<Text
											style={{
										...
										>
											{t('customRangeOrderBlocksExample', {
												defaultValue: 'A1, A2, A3…',
											})}
										</Text>
									</View>
								</TouchableOpacity>
								...
								<TouchableOpacity
									onPress={() => settings.setCustomRangeOrder('ZIP')}
									style={[
										styles.badButton,
										{
											flex: 1,
											backgroundColor:
												settings.customRangeOrder === 'ZIP'
													? colors.accent
													: colors.disabledBg,
											flexDirection: 'row',
											alignItems: 'center',
								...
								>
									...
									<View>
										<Text style={styles.badButtonText}>
											{t('customRangeOrderZipTitle', {
												defaultValue: 'Střídat',
											})}
										</Text>
										<Text
											style={{
							...
							<Text style={[styles.hintText, { marginTop: 8 }]}>
								{t('customRangeOrderHint', {
									defaultValue:
										'Platí pro A1-C4 i A1.1-D1.20. Vyber, jestli se má nejdřív měnit číslo nebo písmeno.',
								})}
							</Text>
						</View>
					</View>
      ...
      <Text>
        ...
        {t('Numeric range (e.g. 1-500 or 1.1-1.20)', { defaultValue: 'Numeric range (e.g. 1-500 or 1.1-1.20)' })}
			...
			<TextInput
				...
				placeholder={t('Numeric range (e.g. 1-500 or 1.1-1.20)', {
					defaultValue: 'Numeric range (e.g. 1-500 or 1.1-1.20)',
				})}
  ...
  <>
    ...
    <Text style={{ color: colors.mutedText, marginBottom: 12 }}>
      {t('exportLangFixedToEnglishFree', { defaultValue: 'Export language is fixed to English in Free version.' })}
    </Text>
  </>
)}
        ...
        {!plusCtx.plus && (
          <Text style={{ color: colors.mutedText, fontSize: 13, marginBottom: 10 }}>
            ...
            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 16, marginBottom: 10 }}>
              <View style={{ flex: 1, minWidth: 130 }}>
                ...
                <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                  ...
                  <TextInput style={[styles.input, { flex: 1, color: colors.text }]} keyboardType="decimal-pad" value={String(minVoltage)} onChangeText={v => handleVoltageInput(v, setMinVoltage)} placeholderTextColor={colors.mutedText} />
                  <Text style={{ color: '#888', width: 22 }}>{t('Max', { defaultValue: 'Max' })}</Text>
                  <TextInput style={[styles.input, { flex: 1, color: colors.text }]} keyboardType="decimal-pad" value={String(maxVoltage)} onChangeText={v => handleVoltageInput(v, setMaxVoltage)} placeholderTextColor={colors.mutedText} />
                </View>
              </View>
            </View>
            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 16, marginBottom: 10 }}>
              <View style={{ flex: 1, minWidth: 130 }}>
                ...
                <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                  <Text style={{ color: '#888', width: 22 }}>{t('Min', { defaultValue: 'Min' })}</Text>
                  <TextInput style={[styles.input, { flex: 1, color: colors.text }]} keyboardType="decimal-pad" value={String(minCurrent)} onChangeText={v => handleCurrentInput(v, setMinCurrent)} placeholderTextColor={colors.mutedText} />
                  <Text style={{ color: '#888', width: 22 }}>{t('Max', { defaultValue: 'Max' })}</Text>
                  <TextInput style={[styles.input, { flex: 1, color: colors.text }]} keyboardType="decimal-pad" value={String(maxCurrent)} onChangeText={v => handleCurrentInput(v, setMaxCurrent)} placeholderTextColor={colors.mutedText} />
                </View>
              </View>
            </View>
            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 16, marginBottom: 10 }}>
              <View style={{ flex: 1, minWidth: 130 }}>
                ...
                <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                  <Text style={{ color: '#888', width: 22 }}>{t('Min', { defaultValue: 'Min' })}</Text>
                  <TextInput style={[styles.input, { flex: 1, color: colors.text }]} keyboardType="decimal-pad" value={String(minResistance)} onChangeText={v => handleResistanceInput(v, setMinResistance)} placeholderTextColor={colors.mutedText} />
                  <Text style={{ color: '#888', width: 22 }}>{t('Max', { defaultValue: 'Max' })}</Text>
                  <TextInput style={[styles.input, { flex: 1, color: colors.text }]} keyboardType="decimal-pad" value={String(maxResistance)} onChangeText={v => handleResistanceInput(v, setMaxResistance)} placeholderTextColor={colors.mutedText} />
                </View>
              </View>
            </View>
            <TouchableOpacity
...
// --- CSV Export PREVIEW functions ---

// Pro typ B (single column)
function previewCSVSingleColumn(content) {
  let lines = content.replace(/^\uFEFF/, '').split(/\r?\n/);
  lines = lines.map(l => l.replace(/^"(.*)"$/, '$1').replace(/""/g, '"'));
  return lines.join('\n');
}

// Pro typ C (multi column)
function previewCSVMultipleColumns(content) {
  // Zjisti aktuální delimiter (podle první řádky, buď ; nebo tabulátor)
  let delim = ';';
  const firstLine = content.replace(/^\uFEFF/, '').split(/\r?\n/)[0];
  if (firstLine && firstLine.includes('\t')) delim = '\t';

  let lines = content.replace(/^\uFEFF/, '').split(/\r?\n/);
  // Odstraň prázdné řádky na konci
  while (lines.length > 0 && lines[lines.length - 1].trim() === '') lines.pop();

  // Zjisti max. šířku buněk pro každý sloupec (kvůli zarovnání)
  let columns = lines.map(line => line.split(delim).map(cell =>
  ...
  columns.forEach(row => {
    row.forEach((cell, i) => {
      const w = cell.length;
      if (!maxWidths[i] || w > maxWidths[i]) maxWidths[i] = w;
    });
  });

  // Sestav řádky zarovnané mezerami
  let result = columns.map(row =>
...
const MainScreen = ({ navigation, route }) => {
  ...
  const { ocrSettings } = useContext(OCRSettingsContext);
    ...
    if (route.params && route.params.ocrResult) {
      ...
      if (res.cableLabel) {
        setCableIndex((labels) => {
					const cableLabels = getCableLabels(
						settings.cableMode,
						settings.cableCount,
						settings.customCableRange,
						settings.numericRange,
  ...
  const handleResistanceInput = (val) => {
    ...
    setResistance(clean);
		...
		const processImage = async (uri, ocrSettings = {}) => {
		try {
			console.log('OCR: original URI:', uri);
			let jpegUri = await getJpegFileUri(uri);

			// === Preprocessing podle ocrSettings ===
			const manipulations = [];
			if (ocrSettings.autoCrop && ocrSettings.cropManual) manipulations.push({ crop: ocrSettings.cropManual });
			if (ocrSettings.rotation) manipulations.push({ rotate: ocrSettings.rotation });
			if (ocrSettings.resizeTo) manipulations.push({ resize: ocrSettings.resizeTo });
			if (ocrSettings.invertColors) console.warn('Expo neumí invertColors – přeskočeno.');
			if (ocrSettings.gamma !== 1.0 || ocrSettings.sharpness !== 1.0) console.warn('Expo neumí gamma ani sharpness – přeskočeno.');
			if (ocrSettings.contrastBoost || ocrSettings.noiseReduction || ocrSettings.perspectiveCorrection) console.warn('Expo neumí contrastBoost/noiseReduction/perspectiveCorrection – přeskočeno.');
			...
			if (!info.exists || info.size < 1000) {
				Alert.alert('OCR chyba', 'Soubor s obrázkem chybí nebo je neplatný.');
				return ['OCR failed'];
			}

			let ocrResult;
			try {
				ocrResult = await textRecognition.recognize(jpegUri);
				console.log('OCR: ML Kit result:', ocrResult);
			} catch (ocrErr) {
				console.error('OCR: recognize failed:', ocrErr);
				ocrResult = { text: '', blocks: [] };
			}

			let resultLines = [];
			if (ocrResult && Array.isArray(ocrResult.blocks)) {
				resultLines = ocrResult.blocks.map(block => block.text).filter(Boolean);
			} else if (ocrResult && typeof ocrResult.text === 'string') {
				resultLines = ocrResult.text.split('\n').map(l => l.trim()).filter(Boolean);
			}
			console.log('OCR: resultLines:', resultLines);
			...
			const cleanedLines = resultLines
				.map(line => {
					let cleaned = postprocessOcrText(line);
					if (ocrSettings.decimalSeparator === ',') cleaned = cleaned.replace(/\./g, ',');
					if (ocrSettings.whitelist) cleaned = cleaned.split('').filter(c => ocrSettings.whitelist.includes(c)).join('');
					return cleaned;
				})
				.filter(l => l.length >= (ocrSettings.minLength || 2));

			let napeti = '', proud = '', odpor = '';
			let voltageFromUnit, currentFromUnit, resistanceFromUnit;

			for (let line of cleanedLines) {
				if (!voltageFromUnit) {
					let m = line.match(/(?:U[=:]?\s*)?([\dO0., ]{2,8})\s*([Vv])\b/);
					if (m) voltageFromUnit = m[1];
				}
				if (!currentFromUnit) {
					let m = line.match(/(?:I[=:]?\s*)?([\dO0., ]{1,7})\s*([Aa])\b/);
					if (m) currentFromUnit = m[1];
				}
				if (!resistanceFromUnit) {
					let m = line.match(/(?:R[=:]?\s*)?([\dO0., ]{1,8})\s*(MΩ|MOhm|MO|M0|M0hm|M0Ω|Mohm)/i);
			...
			if (!napeti || !proud || !odpor) {
				...
				if (allNums && allNums.length > 0) {
					let nNums = allNums.map(asNum).filter(x => x !== null && x > 0 && x < 2000);
					nNums = [...new Set(nNums)].sort((a, b) => a - b);
					if (nNums.length >= 3) {
						const proudHodn = nNums[0];
						const odporHodn = nNums[1];
						const napetiHodn = nNums[nNums.length - 1];
						if (!proud) proud = String(proudHodn);
						if (!odpor) odpor = String(odporHodn);
						if (!napeti) napeti = String(napetiHodn);
					}
				}
			}
			...
			if (!napeti || !proud || !odpor) {
				...
				let allNums = allNumsRaw.map(num => {
					if (num.match(/^\d+\s+\d+$/)) return num.replace(/\s+/, '.');
					return num;
				});
			...
			if (!napeti || !proud || !odpor) {
				if (allNums && allNums.length > 0) {
					...
					if (nNums.length >= 3) {
						let max = Math.max(...nNums);
						let min = Math.min(...nNums);
						let mid = nNums.find(x => x !== max && x !== min) || '';
			...
			const detectSet = new Set(ocrSettings.detectWhat || ['voltage', 'current', 'resistance']);
			...
			setVoltage(detectSet.has('voltage') ? napeti : '');
			setCurrent(detectSet.has('current') ? proud : '');
			setResistance(detectSet.has('resistance') ? odpor : '');
  ...
  const scanValuesFromMeter = async () => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets[0]?.uri) {
        await processImage(result.assets[0].uri, ocrSettings);
      }
    } catch (e) {
      Alert.alert(
  ...
  const onTakePhoto = async () => {
	...
	function isCustomRangeValid(rangeStr) {
		const v = (rangeStr || '').trim();
		if (!v) return false;

		try {
			// Použijeme stejný parser jako v nastavení (podporuje A1-F6, A1.1-D1.20 i kombinace přes čárky)
			const labels = getCustomCableLabels(v, 10000, 200, settings.customRangeOrder);

			// Musí vzniknout aspoň 1 kabel
			if (!labels || labels.length === 0) return false;

			// Bezpečnostní limit (aby někdo nezadal rozsah, co vygeneruje tisíce položek)
			if (labels.length > 500) return false;
    ...
    if (settings.cableMode === 'custom' && plusCtx.plus) {
      if (!settings.customCableRange || !isCustomRangeValid(settings.customCableRange)) {
        Alert.alert(
          ...
          t('customRangeError', { defaultValue: 'Invalid format! Use e.g. A1-F6 or A1.1-D1.20.' })
        );
        return;
      }
    }
      ...
      const exportData = {
        ...
        exportT,
			...
			if (!content || content.trim().length === 0) {
				Alert.alert(
					exportT('Export Error', { defaultValue: 'Export Error' }),
					exportT('Nothing to export', { defaultValue: 'Nothing to export' })
				);
				return;
			}

			setExportPreviewContent(content);
			setExportPreviewMeta({ content, ...meta });
  ...
  const handleRecalculateErrors = async () => {
    ...
    console.log('MSG:', t('errorsRecalculated'));
  };
  ...
  const topBarButtons = [
    ...
    {
      key: 'ocrSettings',
      icon: Platform.OS === 'ios' ? 'scan-circle' : 'settings-outline',
      color: '#18b3b5',
      onPress: () => navigation.navigate('OCRSettings'),
    },
    {
      key: 'saved',
      icon: 'folder-open',
  ...
  return (
    <>
      <View style={{ padding: 20, alignItems: 'center', backgroundColor: colors.surface }}>
      ...
      {!languageSelected && (
        <Modal transparent={true} animationType="slide" visible={!languageSelected}>
          <View style={[styles.languageContainer, { backgroundColor: 'rgba(0, 0, 0, 0.4)' }]}>
            <Animated.View style={{ backgroundColor: colors.surface, padding: 20, borderRadius: 12, width: '80%', transform: [{ scale: languageAnim }] }}>
            ...
            <View>
              ...
              {measuredFields.voltage && (
                <View style={styles.formGroup}>
                  ...
                  <TextInput style={[styles.input, { color: colors.text }]} placeholder={t('Voltage (V)', { defaultValue: 'Voltage (V)' })} placeholderTextColor={colors.mutedText} value={voltage} onChangeText={handleVoltageInput} keyboardType="decimal-pad" returnKeyType="next" onSubmitEditing={() => currentInput.current?.focus()} />
                </View>
              )}
              ...
              {measuredFields.current && (
                <View style={styles.formGroup}>
                  ...
                  <TextInput style={[styles.input, { color: colors.text }]} placeholder={t('Current (A)', { defaultValue: 'Current (A)' })} placeholderTextColor={colors.mutedText} value={current} onChangeText={handleCurrentInput} keyboardType="decimal-pad" returnKeyType="next" onSubmitEditing={() => resistanceInput?.focus()} ref={currentInput} />
                </View>
              )}
              ...
              {measuredFields.resistance && (
                <View style={styles.formGroup}>
                  ...
                  <TextInput style={[styles.input, { color: colors.text }]} placeholder={t('Resistance (MΩ)', { defaultValue: 'Resistance (MΩ)' })} placeholderTextColor={colors.mutedText} value={resistance} onChangeText={handleResistanceInput} keyboardType="decimal-pad" returnKeyType="done" ref={(ref) => (resistanceInput = ref)} onSubmitEditing={() => handleNextCableWrapped()} />
                </View>
              )}
        ...
        <View style={{flex:1, backgroundColor:'#fff', padding:12, paddingBottom:30}}>
              ...
              <View style={{borderBottomWidth:1, borderColor:'#eee', marginBottom:10, paddingBottom:10}}>
                <View style={{flexDirection:'row', alignItems:'center', marginBottom:2}}>
                  <Text>{t('Inverter Number', { defaultValue: 'Inverter Number' })}: </Text>
									<TextInput
											style={[styles.input, { flex: 1, marginVertical: 0, marginHorizontal: 5, padding: 4, fontSize: 15, color: colors.text }]}
                ...
                <View style={{flexDirection:'row', alignItems:'center', marginBottom:2}}>
                  ...
                  <TextInput style={[styles.input, { flex: 1, marginVertical: 0, marginHorizontal: 5, padding: 4, fontSize: 15, color: colors.text }]} value={item.voltage?.toString() || ''} keyboardType="decimal-pad" onChangeText={v => handleEditField(index, 'voltage', v)} editable={plusCtx.plus} returnKeyType="done" placeholderTextColor={colors.mutedText} onSubmitEditing={() => { Keyboard.dismiss && Keyboard.dismiss(); }} />
                  <Text>V</Text>
                </View>
                <View style={{flexDirection:'row', alignItems:'center', marginBottom:2}}>
                  <Text>🔌 {t('Current', { defaultValue: 'Current' })}: </Text>
                  <TextInput style={[styles.input, { flex: 1, marginVertical: 0, marginHorizontal: 5, padding: 4, fontSize: 15, color: colors.text }]} value={item.current?.toString() || ''} keyboardType="decimal-pad" onChangeText={v => handleEditField(index, 'current', v)} editable={plusCtx.plus} returnKeyType="done" placeholderTextColor={colors.mutedText} onSubmitEditing={() => { Keyboard.dismiss && Keyboard.dismiss(); }} />
                  <Text>A</Text>
                </View>
                <View style={{flexDirection:'row', alignItems:'center', marginBottom:2}}>
                  <Text>🛡️ {t('Resistance', { defaultValue: 'Resistance' })}: </Text>
                  <TextInput style={[styles.input, { flex: 1, marginVertical: 0, marginHorizontal: 5, padding: 4, fontSize: 15, color: colors.text }]} value={item.resistance?.toString() || ''} keyboardType="decimal-pad" onChangeText={v => handleEditField(index, 'resistance', v)} editable={plusCtx.plus} returnKeyType="done" placeholderTextColor={colors.mutedText} onSubmitEditing={() => { Keyboard.dismiss && Keyboard.dismiss(); }} />
                  <Text>MΩ</Text>
                </View>
                <View style={{flexDirection:'row', alignItems:'center', marginBottom:2}}>
                  <Text>{t('Note', { defaultValue: 'Note' })}: </Text>
                  <TextInput style={[styles.input, { flex: 1, marginVertical: 0, marginHorizontal: 5, padding: 4, fontSize: 15, color: colors.text }]} value={item.note || ''} onChangeText={v => handleEditField(index, 'note', v)} editable={plusCtx.plus} returnKeyType="done" placeholderTextColor={colors.mutedText} onSubmitEditing={() => { Keyboard.dismiss && Keyboard.dismiss(); }} />
                </View>
          ...
          <TouchableOpacity style={[styles.badButton, {marginTop:10, backgroundColor:colors.primary}]} onPress={copyData}>
			...
			<Modal visible={showOverlayModal} animationType="fade" transparent={true}>
				<View style={{
					...
					<View style={{
						...
						<TextInput style={[styles.input, { width: '95%', marginBottom: 8, textAlign: 'center', color: colors.text }]} value={overlayLabel} onChangeText={setOverlayLabel} autoCapitalize="characters" maxLength={30} placeholderTextColor={colors.mutedText} />
							...
							<TouchableOpacity
								...
								onPress={() => {
									if (overlayModalCallbackRef.current) {
										overlayModalCallbackRef.current(pendingPhoto?.uri, overlayLabel);
									}
								}}
							>
								<Text style={styles.badButtonText}>{t('Save', { defaultValue: 'Save' })}</Text>
			...
			>
				...
				>
					...
					>
						...
						>
							{exportPreviewMeta.extension === '.txt'
								? t('Preview TXT export', { defaultValue: 'Preview TXT export' })
								: t('Preview CSV export', { defaultValue: 'Preview CSV export' })}
						</Text>
            ...
            <ExportPreview style={{
              ...
              {exportPreviewMeta.extension === '.csv' && exportPreviewMeta.content
                ? (
                  exportPreviewMeta.content.indexOf(';') !== -1
                    ? previewCSVMultipleColumns(exportPreviewMeta.content)
                    : previewCSVSingleColumn(exportPreviewMeta.content)
                )
						...
						<View style={{ flexDirection: 'row', gap: 14 }}>
							<TouchableOpacity
								...
								onPress={async () => {
									...
									await exportDataToFile(exportPreviewMeta);
								}}
							>
								<Text style={styles.badButtonText}>
      ...
      <Modal visible={filenamePromptVisible} animationType="fade" transparent={true}>
        <View style={{
          ...
          <View style={{
            ...
            <TextInput style={[styles.input, { width: '95%', marginBottom: 12, color: colors.text }]} value={filenamePromptValue} autoFocus onChangeText={setFilenamePromptValue} placeholder={t('File name', { defaultValue: 'File name' })} placeholderTextColor={colors.mutedText} onSubmitEditing={() => { setFilenamePromptVisible(false); filenamePromptCb(filenamePromptValue); }} />
            <View style={{ flexDirection: 'row', gap: 10 }}>
              <TouchableOpacity
                style={[styles.badButton, { backgroundColor: colors.primary, flex: 1 }]}
                onPress={() => {
          ...
          <OCRSettingsProvider>
            <NavigationContainer>
              ...
              >
                ...
                <Stack.Screen name="OCRSettings" component={OCRSettingsScreen} />
              </Stack.Navigator>
            </NavigationContainer>
          </OCRSettingsProvider>
        </PlusProvider>
